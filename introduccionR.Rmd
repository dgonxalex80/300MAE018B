...
4. **Pide ayuda**: usa `?funcion` o `help("funcion")` para leer la documentación.
...
plot(x, fx(x), type="l", col=c1, las=1) # genera gráfica
```
...
grafica(z,c1)  # evalúa la función en los datos z y color rojo
```


```{r}
w=round(prop.table(z)*100,2)  # genera datos y construye tabla en porcentaje
grafica(w,c6)  # evalúa la función con w y color azul
```
