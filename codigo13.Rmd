...
pie(t1, col = cols_t1[1:length(t1)])
```
...
cols_ev <- if (length(ev) <= 4) paleta4 else paleta
barplot(ev,  col = cols_ev[1:length(ev)], 
...
cols_counts <- if (nrow(counts) <= 4) paleta4 else paleta5
barplot(counts, main = "Método de pago por tipo de cliente",  
        xlab = "Tipo de cliente",
        col = cols_counts[1:nrow(counts)],
...
cols_counts <- if (nrow(counts) <= 4) paleta4 else 5
barplot(counts, main = "Método de pago por tipo de cliente",  
        xlab = "Tipo de cliente",
        col = cols_counts[1:nrow(counts)],
        legend = rownames(counts),
...
p <- ggplot(resumen, aes(x = mes, y = Ventas_Netas, color = Tipo_Cliente, group = Tipo_Cliente)) +
  ...
  geom_point(size = 2) +
  scale_color_manual(values = paleta5) +   # paleta institucional (5 colores)
  labs(
...
library(gganimate)
library(gifski)   # si vas a exportar gif
gif <- gganimate::animate(p, fps = 20, width = 900, height = 500,
                          renderer = gifski_renderer())
...
if (is.null(cali)) { # valida que se haya cargado
  ...
  plot(sf::st_geometry(cali), col = c7, border = c1, # geometría y colores
...
spplot(mapco["indicador"], col.regions = mi.palette, cuts = 6)  # mapa
```
...
document.getElementById('dl-practica').addEventListener('click', async () => {
  const urlRaw = 'practica_beer.R';
  const resp = await fetch(urlRaw);
  const blob = await resp.blob();
  const url  = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = 'practica_beer.R';
  document.body.appendChild(a);
  a.click();
  a.remove();
  URL.revokeObjectURL(url);
